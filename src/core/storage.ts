...
export function migrateProfile(p: Partial<Profile>): Profile {
  const np: Profile = {
    ...
    // Badge system fields
    badges: p.badges || { unlocked: {}, counters: {} },
    streakDays: typeof p.streakDays === 'number' ? p.streakDays : 0,
    sessionDamageTaken: typeof p.sessionDamageTaken === 'number' ? p.sessionDamageTaken : 0,
    dayCounters: p.dayCounters || {
      solved: 0,
      solvedMonth: 0,
  ...
  if (np.skillStats) {
    Object.keys(np.skillStats).forEach(skillId => {
      const stat = (np.skillStats as any)[skillId];
      if (stat && !stat.responseTimes) {
        // For existing profiles, initialize empty responseTimes array
        // They'll build up new data as they play
        stat.responseTimes = [];
      }
    });
  }
  ...
  if (!np.unlocks.skins.includes(np.settings.activeSkin)) {
    if (!np.unlocks.skins.includes('moss')) np.unlocks.skins.push('moss');
    np.settings.activeSkin = 'moss';
  }
  return np;
}
